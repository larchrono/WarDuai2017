...
public class WorldCameraAndUserInput : MonoBehaviour {
	...
	void Update () {
		...
		// look at main Actor when in world and InWorld
		if (GameCamera.IsAimCharactor && (GlobalData.Instance.GameInState == GlobalData.GameStates.IN_WORLD || GlobalData.Instance.GameInState == GlobalData.GameStates.IN_TALK)) {
		...
		if (Input.GetButton ("L2")) {
			...
			if (camXRotate < camXRotateMin)
				camXRotate = camXRotateMin;
		}
		if (Input.GetButton ("R2")) {
			camXRotate += camYRotateSpeed * Time.deltaTime;
			if (camXRotate > camXRotateMax)
				camXRotate = camXRotateMax;
		}
		...
		// Allow user to input when in World
		if (GlobalData.Instance.GameInState == GlobalData.GameStates.IN_WORLD) {
			...
			targetActor.GetComponent<WorldActorAct> ().Move (Vector3.zero);
		}
	}
