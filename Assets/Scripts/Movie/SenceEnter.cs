...
public class SenceEnter : MonoBehaviour {
	...
	IEnumerator Start () {
		...
		Transmission.FromUnit (GlobalData.Instance.ActiveActors[0], "荷魯斯", "好痛啊~", "Cry", 0, true, 5, delegate() {
		...
		Transmission.FromUnit (MonsterDataBase.MonsterData[2], "少年", "阿米特，那個還不能吃喔", "Stand", 1, false, 5);
		Transmission.FromUnit (MonsterDataBase.MonsterData[3], "阿米特", "(哈哈喘氣)", "Stand", 0, false, 5);

		Transmission.FromUnit (GlobalData.Instance.ActiveActors[0], "荷魯斯", "你是這裡的居民嗎?", "Stand", 0, false, 5);
		Transmission.FromUnit (MonsterDataBase.MonsterData[2], "少年", "我才不住貧民窟呢!只是帶阿米特來這裡散步!", "Stand", 0, false, 5);
		Transmission.FromUnit (GlobalData.Instance.ActiveActors[0], "荷魯斯", "哦?那你知道這裡的宮殿該往哪裡走嗎?", "Stand", 0, false, 5, delegate() {
			isAnuMove = true;
			actorAnubis.GetComponent<Animator> ().CrossFade ("walking",0.5f);
			actorAmit.GetComponent<Animator> ().CrossFade ("walk",0.5f);
		});
		Transmission.FromUnit (MonsterDataBase.MonsterData[2], "少年", "嗯~現在我沒空跟你聊天。哈哈哈阿米特我們繼續走~", "Stand", 0, false, 5, delegate() {
			actorNaphy.GetComponent<Animator> ().CrossFade ("leg_sweep", 0.25f);
			GameCamera.ApplyCameraObject (camera_enter1, 2.5f);
		});

		Transmission.FromUnit (GlobalData.Instance.ActiveActors [1], "奈芙蒂斯", "站住!不准你走!", "Stand", 0, false, 5, delegate() {
			isMovieFinish = true;
		});
		...
		SceneManager.LoadSceneAsync ("BattleSence");
