...
public class UIActorData : MonoBehaviour {
	...
	public void SetupData(StandardActor data){
		...
		if (EXP_Bar != null){
			_exp_base = (float)GlobalData.Instance.GetThisLevelBaseExp (UIUSE_level);
			_exp_end  = (float)GlobalData.Instance.GetToNextEXP (UIUSE_level);
	...
	void CalcuExpBarLocation(){
		if (EXP_Bar != null) {
			...
			if (_expbar_now > 1.0f) {
				...
				_exp_base = GlobalData.Instance.GetThisLevelBaseExp (UIUSE_level);
				_exp_end = GlobalData.Instance.GetToNextEXP (UIUSE_level);
				_expbar_now = (_exp_now - _exp_base) / (_exp_end - _exp_base);
			}
